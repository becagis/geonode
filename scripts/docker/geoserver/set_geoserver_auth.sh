...
do
    ...
    case $i in
        ...
        userAuthorizationUri | redirectUri | logoutUri )
            ...
            newvalue=`echo -ne "$tagvalue" | sed -re "s@http://localhost(:8.*0)@$SUBSTITUTION_URL@"`;;
        scopes )
            echo "DEBUG: Editing '$auth_conf_source' for tagname <$i>"
            newvalue=`echo -ne "$tagvalue" | sed -re "s@write@write,read,groups@"`;;
        *) echo -n "an unknown variable has been found";;
    esac

    if [ ${GEOSERVER_USE_SSL} ]; then newvalue=`echo -ne "$newvalue"| sed "s/http:\/\//https:\/\//g"`; fi;
