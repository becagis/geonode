...
def get_user_geolimits(layer, user, group, gf_services):
    ...
    if user:
        ...
        _disable_layer_cache = users_geolimits.exists()
    ...
    if group:
        ...
        if GroupProfile.objects.filter(group__name=_group).count() == 1:
            ...
            _disable_layer_cache = groups_geolimits.exists()
    ...
    if not user and not group:
        ...
        _disable_layer_cache = anonymous_geolimits.exists()
...
def sync_permissions_and_disable_cache(cache_rules, resource, perms, user, group, group_perms):
    if group_perms:
        sync_geofence_with_guardian(layer=resource, perms=perms, user=user, group_perms=group_perms)
    else:
        sync_geofence_with_guardian(layer=resource, perms=perms, user=user, group=group)
    gf_services = _get_gf_services(layer=resource, perms=perms)
    _, _, _disable_layer_cache, _, _, _ = get_user_geolimits(layer=resource, user=user, group=group, gf_services=gf_services)
    cache_rules.append(_disable_layer_cache)
    return list(set(cache_rules))


def remove_permissions(instance=None) -> bool:
    from geonode.layers.models import Layer
